...
if (!settings.manRoles || !settings.vipRole || !settings.boosterRole || !settings.registerAuthorizedRoles) return message.channel.send(embed.setDescription("Sunucu ayarları kurulmamış." + "`" + conf.prefix + "ayar`")).then(qwe => qwe.delete({timeout: 3 * 1000}));
...
qwe.roles.set(settings.manRoles || []).catch();
message.channel.send(embed.setDescription(`:tada: ${qwe} kullanıcısına başarıyla ${settings.manRoles.map(qwe => `<@&${qwe}>`).join(", ")} rolleri verildi.`)).then(qwe => qwe.delete({timeout: 15 * 1000}));
} else {
qwe.setNickname(name);
qwe.roles.set(settings.manRoles || []).catch();
message.channel.send(embed.setDescription(`:tada: ${qwe} kullanıcısına başarıyla ${settings.manRoles.map(qwe => `<@&${qwe}>`).join(", ")} rolleri verildi.`)).then(qwe => qwe.delete({timeout: 15 * 1000}));
