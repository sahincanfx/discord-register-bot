...
module.exports = message => {
  ...
  let embed = new MessageEmbed().setColor("RANDOM").setFooter(config.BotFooter || "Reawen ❤️ Arox").setTimestamp().setAuthor(message.member.displayName, message.guild.iconURL({ dynamic: true }));
  if (cmd) {
    if(!message.guild) {
   ...
   if(!config.geliştiriciler.includes(message.author.id)) {
    ...
    cmd.run(client, message, params, embed, settings);
};
};